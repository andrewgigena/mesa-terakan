...
RatInstr::emit_uav_instr(nir_intrinsic_instr *intrin, Shader& shader)
{
   ...
   auto coord = vf.temp_vec4(
      pin_chgr, num_components_swizzles[coord_num_components - 1]);
   for (unsigned i = 0; i < coord_num_components; ++i) {
      shader.emit_instruction(new AluInstr(
         op1_mov, coord[i], vf.src(intrin->src[1], i),
   ...
   assert(intrin->intrinsic == (op_returns
                                   ? nir_intrinsic_uav_returning_instr_r600
                                   : nir_intrinsic_uav_instr_r600));

   PVirtualValue return_value_index =
      op_returns ? vf.src(intrin->src[4], 0) : nullptr;
   ...
   if (op == ERatOp::STORE_TYPED) {
      ...
      value = vf.temp_vec4(pin_chgr,
                           num_components_swizzles[value_num_components - 1]);
      for (unsigned i = 0; i < value_num_components; ++i) {
         shader.emit_instruction(new AluInstr(
            op1_mov, value[i], vf.src(intrin->src[2], i),
      ...
      if (op_non_rtn != ERatOp::NOP) {
         ...
         if (op_non_rtn == ERatOp::CMPXCHG_INT ||
             op_non_rtn == ERatOp::CMPXCHG_FLT ||
             op_non_rtn == ERatOp::CMPXCHG_FDENORM) {
            value_component_mask |=
               BITFIELD_BIT(compare_value_component_index);
         }
      }
      ...
      unsigned value_last_component_index =
         util_last_bit(value_component_mask) - 1;

      /* Value. */
      if (value_component_mask & BITFIELD_BIT(0)) {
         shader.emit_instruction(new AluInstr(
            op1_mov, value[0], vf.src(intrin->src[2], 0),
      ...
      if (value_component_mask & BITFIELD_BIT(1)) {
         shader.emit_instruction(new AluInstr(
            op1_mov, value[1], return_value_index,
            value_last_component_index == 1
               ? AluInstr::last_write
               : AluInstr::write));
      }
      /* Value to compare to. */
      if (value_component_mask &
             BITFIELD_BIT(compare_value_component_index)) {
         shader.emit_instruction(new AluInstr(
            op1_mov, value[compare_value_component_index],
            vf.src(intrin->src[3], 0),
            value_last_component_index == compare_value_component_index
               ? AluInstr::last_write
               : AluInstr::write));
      }
   }
   ...
   unsigned immed_base =
      op_returns ? nir_intrinsic_uav_return_id_base_r600(intrin) : 0;
   PRegister uav_offset = nullptr;
   const nir_const_value *uav_offset_const =
